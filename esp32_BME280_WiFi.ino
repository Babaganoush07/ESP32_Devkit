...
float getHeatIndex(float temp, float rh){
  ...
  // The simple formula is computed first:
  hi = 0.5 * (temp + 61.0 + ((temp-68.0)*1.2) + (rh*0.094));

  // And the result is averaged with the temperature
  hi = (hi + temp) / 2;

  // if the HI is higher than 80f use the full equation with adjustments
  if (hi > 80.0){
    ...
    // If the rh is less than 13%
    // and temp is between 80f and 112f
    // Then the following is subtracted from HI
    if(rh < 13.0 and temp > 80.0 and temp < 112.0){
      hi = hi - ((13-rh)/4)*sqrt((17-abs(temp-95.))/17);
    // If rh is greater than 85%
    // and temp is between 80f and 87f
    // Then the following is added to HI
    }else if(rh > 85.0 and temp > 80.0 and temp < 87.0){
